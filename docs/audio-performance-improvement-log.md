...
## 5. 2026-06-10 ETA 표시 개선 메모

### 변경 내용

- STT 첫 청크가 끝나기 전에도 파일 길이와 실행 장치 기준의 보수 ETA를 내려보내도록 했다.
- 청크 duration이 없는 경우 마지막 청크 길이를 평균값으로 대체하지 않고 `source_duration - offset`으로 계산하게 했다.
- STT가 실제로 새로 인식한 청크의 오디오 길이/실행 시간을 `analysis_eta_metrics.json`에 저장하고, 다음 분석의 첫 ETA부터 같은 STT 실행 fingerprint(모델 경로, 장치, 청크 길이, 파이프라인 버전)의 최근 처리 속도를 반영하게 했다. 캐시 재사용 청크는 속도 측정에서 제외한다.
- ETA 기준은 파일 크기보다 실제 음성 길이와 최근 처리 속도를 우선한다. 파일 크기는 저장 공간, 업로드, 원본 보존 위험 판단에는 유용하지만 STT/참석자 구분 처리 시간의 1차 신호로 쓰기에는 압축률 차이가 크다.
- 참석자 구분 수동 실행 progress에도 원본 음성 길이 기반 ETA를 포함했다.
- 기본 분석 중 참석자 구분을 실행하는 설정에서도 시작 progress에 원본 음성 길이 기반 ETA를 포함했다.
- 프론트는 참석자 구분 ETA가 있으면 진행률/경과시간 추정보다 backend ETA를 우선 표시한다.
- backend가 ETA를 알 수 없는 단계와 취소/실패 같은 종료 상태는 `eta_seconds: null`을 명시해 오래된 ETA가 남지 않게 했다.
- `MeetingWriter`의 backend ETA는 수신 시각 기준으로 화면에서 초 단위 countdown 되도록 했다.
- countdown 결과가 15초 미만이면 진행 중 화면에서 `약 0:00` 대신 `곧 완료`로 표시한다.

### 남은 검증

- 실제 긴 다화자 샘플에서 첫 실행의 fallback 계수(STT CPU 기준 `0.25 audio sec/sec`, 참석자 구분 `duration * 1.5`)가 과도하게 보수적이거나 낙관적인지 확인해야 한다.
- 30% 참석자 음성 구간 분석 단계는 실제 진행률 절반이 아니라 긴 blocking 단계 시작에 가깝다. ETA가 소진돼도 70% 이전에는 `곧 완료`로 표시하지 않고 `계산 중`으로 되돌린다.
- 별도 참석자 구분 화면은 backend가 30% 단일 이벤트만 보내는 동안에도 elapsed 기반 추정 진행률을 68%까지 천천히 보여준다. 실제 내부 진행률이 아니라 "멈춰 보임"을 줄이는 UI 보정이며, backend가 78% 이상 이벤트를 보내면 실제 단계 진행률로 전환한다.
- 분석 작성 화면에도 `대화록 생성 후 참석자 구분까지 이어서 실행` 토글을 노출해, 결과 화면에서 따로 실행하는 흐름과 분석 중 이어서 실행하는 흐름을 사용자가 시작 전에 선택하게 했다.
- 같은 PC에서 2회차 이후 STT ETA가 `analysis_eta_metrics.json`의 최근 속도를 반영해 초반에 덜 흔들리는지 확인한다.
- 음성 추출/변환은 현재 단계 구분과 `eta_seconds: null` 계약으로 오래된 ETA를 지우지만, ffmpeg 진행률을 파싱한 변환 ETA는 아직 구현하지 않았다. 추출 단계가 실제 샘플에서 길게 걸리면 별도 작업으로 `-progress` 기반 측정을 추가한다.
- 목표는 초반 ETA가 계속 늘어나는 느낌을 줄이는 것이며, 실제 샘플 결과에 따라 fallback 계수를 조정한다.
...
## 2026-05-18 별도 발화자 구분 원본 복구

- 확인한 문제: 별도 발화자 구분은 `backend/temp/jobs/{job_id}/audio/source.wav`에 의존한다. 샘플 기록처럼 회의 기록과 대화록은 남았지만 체크포인트 WAV가 없거나, 설정상 추출 WAV가 삭제되면 `audio_required_for_diarization`으로 실패한다.
- 조치: 별도 발화자 구분 실행 시 기존 `source.wav`를 우선 사용하고, 없을 때만 보존된 업로드 원본 또는 허용된 샘플/배포 폴더의 원본 파일명으로 원본을 찾아 `source.wav`를 다시 만든다. 복구 후보는 명시 루트와 미디어 확장자로 제한한다. 업로드 원본은 기록된 원본 파일명에서 나온 `source.{확장자}`만 허용하고, 업로드 폴더 전체를 임의 검색하지 않는다.
...
## 2026-06-10 분석 ETA와 참석자 구분 기본 흐름 조정

- 기본 흐름은 `diarization.generate_during_analysis=true`로 두어 대화록 생성 뒤 참석자 구분까지 이어서 실행한다. 다만 긴 파일은 전체 완료까지 늦어질 수 있으므로 분석 화면과 설정 화면에서 이 영향을 직접 안내한다.
