...
대화록 작성에 필요한 기본 음성 인식 모델과 참석자 구분 모델은 앱 안에서 다운로드하지 않습니다. 관리자가 준비한 파일을 `models` 폴더에 넣는 방식입니다.

전체 요약, 주제별 정리, 참석자별 정리에 쓰는 Ollama 정리 모델은 별도입니다. 외부망이 있고 Ollama가 준비된 PC에서는 시스템 설정 > 모델에서 추천 정리 모델을 받거나 직접 입력한 모델을 선택할 수 있습니다. 내부망 또는 오프라인 PC에서는 이미 설치된 Ollama 모델이 감지되는지 확인하고, 모델이 없으면 대화록 작성만 먼저 사용합니다.
