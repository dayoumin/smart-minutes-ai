...
3. 설정 > 모델에서 기본 음성 인식 모델과 참석자 구분 모델 상태를 확인한다.
4. STT와 참석자 구분 모델은 앱 안에서 받는 대상이 아니다. 회사 전달용 슬림 zip은 참석자 구분 모델을 포함하고 STT용 `faster-whisper-large-v3`만 제외하므로, 누락 시에는 먼저 어떤 모델이 빠졌는지 확인한 뒤 관리자가 `models` 폴더에 복사하고 새로고침한다.
