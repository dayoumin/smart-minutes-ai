...
프론트엔드 시뮬레이션과 `check:quick`/`check:release`는 UI 상태 전이와 API 계약을 확인한다. 다른 PC에서 portable 앱이 실제로 실행되는지, Ollama 정리 모델을 받을 수 있는지, 사용자가 다운로드한 결과물이 열리는지는 별도 수동 검증으로 본다.

다른 PC로 가져가기 전 기준:

- 변경 사항을 커밋한 뒤 `corepack pnpm build`로 `releases\lmo_audio`를 새로 만든다.
- `release-manifest.json`의 `commit`이 현재 HEAD와 같고 `dirty=false`인지 확인한다.
- 회사 PC에서 직접 빌드할 경우에는 최신 `origin/main`을 pull한 뒤 회사 PC에서 같은 빌드와 검증을 다시 한다.
- 이미 만들어진 portable을 옮길 경우에는 `lmo_audio.exe`만 복사하지 않고 `releases\lmo_audio` 폴더 전체를 옮긴다.

다른 PC에서 확인할 최소 흐름:

1. 쓰기 가능한 위치에 `lmo_audio` 폴더 전체를 둔다.
2. `lmo_audio.exe` 실행 시 콘솔 창이 뜨지 않고 앱이 열린다.
3. 설정 > 모델에서 기본 음성 인식 모델과 회의 요약 모델 상태를 확인한다.
4. STT 모델은 외부망 PC에서 설정 화면의 `받기`로 받을 수 있다. 회사 전달용 슬림 zip은 앱 실행용 소형 모델을 포함하고 STT용 `faster-whisper-large-v3`만 제외하므로, STT 다운로드가 실패하거나 내부망이면 관리자가 `models\faster-whisper-large-v3`에 복사하고 새로고침한다.
5. 외부망과 Ollama가 준비된 PC에서는 설정 > 모델에서 추천 정리 모델 또는 직접 입력 모델을 받아 보고, 완료 후 사용 중 모델로 선택되는지 확인한다.
6. 내부망/오프라인 PC에서는 이미 설치된 Ollama 모델이 감지되는지 확인하고, 모델이 없으면 대화록 작성만 완료되는지 본다.
7. 짧은 MP4 또는 WAV로 대화록 작성이 완료되고 회의 기록에 저장되는지 확인한다.
8. 원본 음성이 보존된 상태에서 참석자 구분 실행 가능 여부와 실패 안내를 확인한다.
9. Ollama 정리 모델이 준비된 경우 전체 요약, 주제별 정리, 참석자별 정리를 각각 실행한다.
10. HWPX/TXT/DOCX 다운로드를 실행하고, 특히 HWPX는 한글 또는 HWPX 뷰어에서 실제로 열린다는 것을 확인한다.
11. 영상에서 음성 파일 저장을 실행했다면 사용자가 보는 다운로드 위치와 앱 내부 `backend\outputs` 위치가 혼동되지 않는지 확인한다.
12. 앱을 닫고 다시 열었을 때 회의 기록과 다운로드 가능한 결과가 유지되는지 확인한다.

이 검증은 릴리스 후보나 회사 PC 인수 전에는 생략하지 않는다. 자동 테스트가 통과했더라도 다른 PC 수동 검증 전에는 "회사 PC 사용 가능"으로 완료 처리하지 않는다.
