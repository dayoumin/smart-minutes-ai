# 데스크톱 완성 후 웹 로컬 엔진 계획

- 결정일: 2026-07-28
- 상태: 후속 제품 방향 확정, 구현 대기
- 착수 조건: Windows Tauri 데스크톱 앱의 실제 실행·portable 검증 완료
...
웹 구현은 다음 조건이 모두 확인된 뒤 시작한다.

1. 1440/1280px 기본 데스크톱과 1024px 최소 지원 창 UI 검수가 끝난다.
2. typecheck, lint, 관련 Playwright 시뮬레이션이 통과한다.
3. 실제 releases/lmo_audio/lmo_audio.exe에서 음성·영상 분석, 중지·이어하기, 선택형 참석자 구분, 요약, 내보내기를 확인한다.
4. verify_portable.ps1와 사용자 전달용 handoff 패키지 검증이 통과한다.
5. 모델 다운로드·중지·이어받기·해시 검증과 앱 관리 Ollama 경로가 실제 Windows PC에서 통과한다.
6. 저장 계약과 SQLite 전환 범위를 확정한다.

브라우저에서 React 화면이 열린다는 사실만으로 이 게이트를 통과한 것으로 보지 않는다.
