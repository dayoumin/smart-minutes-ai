...
- 최초 결정일: 2026-07-28
- 우선순위 변경일: 2026-08-13
- 상태: 첫 제품 구현 방향, 웹 MVP 우선 착수
- 구현 원칙: 웹 UI를 먼저 제공하고 분석·저장·모델 실행은 사용자 PC의 로컬 엔진을 우선한다.
...
웹 구현은 Windows Tauri portable 완성을 선행 조건으로 두지 않는다. 공통 React UI와 현재 네이티브 분석 코드를 재사용하되, 다음 로컬 실행 경계를 웹 MVP의 착수·완료 기준으로 삼는다.

1. HTTPS 웹 UI와 127.0.0.1 고정 포트 로컬 엔진의 health·single instance·재연결 PoC가 동작한다.
2. 음성·영상 원본, 대화록, 요약과 사용자 설정은 기본적으로 원격 분석 서버에 전송하지 않는다.
3. exact origin allowlist, Local Network Access, 설치별 pairing과 API 인증 경계를 확정한다.
4. 엔진·모델 다운로드의 버전, 크기, SHA-256, 중지·이어받기·복구 계약을 확정한다.
5. 로컬 엔진의 SQLite와 결과 폴더를 영구 기록의 단일 원본으로 사용하는 저장 계약을 확정한다.
6. 공통 React UI의 typecheck, 변경 파일 lint, 관련 시뮬레이션과 Chrome/Edge 재접속 흐름을 확인한다.

브라우저에서 React 화면만 열린 상태나 원격 서버가 대신 분석하는 상태는 웹 MVP 완료로 보지 않는다. 실제 로컬 엔진 연결, 로컬 처리, 로컬 저장과 다운로드 복구까지 확인해야 한다.

## 8. 웹 구현 순서

### 0단계: 로컬 엔진 분리 PoC

- 현재 Tauri launcher와 Python sidecar에서 창 없는 로컬 엔진 패키지를 분리한다.
...
- 공통 React UI와 저장·내보내기 계약의 데스크톱 회귀 확인
- 실제 portable `lmo_audio.exe`의 sidecar·파일 시스템·재시작·종료 guard 검증
- 웹 MVP에서 검증한 로컬 엔진·모델 다운로드 계약을 데스크톱 제공 경로에도 재사용
- portable/handoff 배포는 웹 MVP를 막는 선행 게이트가 아니라 별도 후속 릴리스로 관리
