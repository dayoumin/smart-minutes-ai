...
- [ ] **회의 기록 아카이브 고도화**: 즐겨찾기, 휴지통, 태그, 담당자·상태 필터와 결정 변경 이력을 SQLite 전환 이후 추가한다.
- [ ] **근거 기반 질문(RAG)**: "지난번 회의에서 A를 언제 결정했지?" 같은 질문에 원래 회의, 결정 문장, 발언 시각을 함께 제시한다. 근거 없는 답변은 생성하지 않는다.
- [ ] **팀 프로젝트 및 히스토리 관리**: 팀 단위 권한 관리 및 히스토리 추적
- [ ] **외부 협업 툴 연동**: Notion, Slack, Jira 자동 연동

## 📄 내부망 문서 활용 후속
- [ ] **HWPX 문서 생성**: 사용자가 확인한 회의 요약·결정·할 일을 선택해 보도자료, 결과보고서, 기관 회의록 템플릿에 매핑한다. 기록 찾기 화면은 신뢰 가능한 원문 선택과 출처 제공을 담당한다.
- [ ] **기관 문서 템플릿 관리**: HWPX/DOCX에 기관별 제목, 결재선, 표, 본문 구조 프리셋을 적용한다.
- [ ] **내부망 HWPX 편집기**: 문서 생성과 템플릿 흐름이 안정된 뒤 별도 작업으로 편집, 미리보기, 저장, 재열기 범위를 설계한다. 현재 기록 찾기 MVP에는 편집기를 포함하지 않는다.
- [ ] **보도자료 작성 보조**: 확정된 결정과 근거만 입력으로 사용하고, 초안·검토 필요·확정 상태를 구분하며 원 회의록으로 되돌아갈 수 있게 한다.
