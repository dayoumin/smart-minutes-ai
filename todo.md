...
- [x] 3-2순위: 회의 요약 / 대화록 디자인 시스템 2차 정리
  ...
  - 2026-06-05 완료: `새 회의록 작성` 첫 화면에서 새 회의록 작성 폼이 먼저 읽히도록 미완료/이전 분석 기록 패널의 화면 순서를 낮췄다.
  - 2026-06-05 완료: 사이드바 `미완료 분석 기록`에 semantic status token과 `status-pill`을 적용해 `진행 중`, `이어하기 가능`, `취소/실패`, `이어하기 불가` 상태가 같은 규칙으로 보이게 했다.
