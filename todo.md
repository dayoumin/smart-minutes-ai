...
- [ ] 2026-07-29 바로록 데스크톱·웹 제품 전환
  ...
  - 다음 단계: 승인된 프로덕션 시각 기준 구현과 화면 단위 검증을 마친 뒤 실제 `releases\lmo_audio\lmo_audio.exe`를 기준으로 portable 데스크톱 게이트를 수행한다. sidecar 시작, 모델 경로, 음성·영상 파일 선택과 분석, 중지·이어하기, 참석자 구분, 요약, 저장·폴더 열기, 내보내기, backend 재시작, 종료 guard를 확인한 뒤에만 웹 로컬 엔진 단계로 넘어간다.
